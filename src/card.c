...
void drawCardContent(Card *card, SDL_Rect cardRect) {
    ...
    if (!card || !card->texture || !card->texture->texture) return;
    ...
    if (cardTexture->alignment) { // Vertical alignment
...
void getCardColor(const Card* card, SDL_Color* color) {
    if (card) *color = (SDL_Color){70, 70, 70, 255};
}

char *cardTypeToString(CardType type) {
    switch (type) {
        case CARD_ENEMY: return "Enemy";
        case CARD_WEAPON: return "Weapon";
        case CARD_SHIELD: return "Shield";
        case CARD_POTION: return "Potion";
        case CARD_COIN: return "Coin";
